...
// Softwarezustände der Taster: hoch, runter
...
{
  uint32_t now = millis();
  if (now - lastLuefterOffTime < DEBOUNCE_MS)
    return;
...
void tasterHochRunterVerarbeiten()
{
  ...
  {
    if (upReleaseTime == 0)
      upReleaseTime = millis();
    else if (millis() - upReleaseTime >= RELEASE_STABLE_MS)
      upArmed = true;
  }
  ...
  {
    if (downReleaseTime == 0)
      downReleaseTime = millis();
    else if (millis() - downReleaseTime >= RELEASE_STABLE_MS)
      downArmed = true;
  }
}
...
void setup()
{
  ...
  // Initialwert setzen: Display: Balkenanzeige und 7‑Segment‑Anzeige.
  sendeDaten(0, 0);

  // Pins der Statusanzeige
...
// Das ist die Funktion, die die Befehle der vier Tasten verarbeitet, die in den Mikrocontroller gelangen vom Webinterface
...
void loop()
{
  ...
  processIncomingSerial(); // Rufe die serielle Eingabe auf: Sind Befehle vom Web-Interface eingetroffen?

  tasterHochRunterVerarbeiten(); // Taster gedrückt, physisch hoch/runter
  if (aktueller_zustandx == LUEFTER_EIN)
  {
    ...
    if (aktueller_zustandy == LUEFTER_STAGE_UP)
    {
      ...
      if (button_PWM_stage < 4)
      { // Erlaubt das Hochzählen nur bis max. 4
        button_PWM_stage++;
    ...
    {
      if (button_PWM_stage > 0)
      { // Erlaubt das Runterzählen nur bis min. 0
        button_PWM_stage--;
    ...
    case 1: // Erste Langsame Stufe mit 800ms Anlaufboost
      if (!boostAktiv)
      {
        ...
        ledcWrite(OUTPUT_PWM_PIN, 0); // Vollgas für Anlauf um Drehmoment zum Anlaufen zu erreichen (invertiert)
        currentDutyCycle = 0;
      ...
      {
        ledcWrite(OUTPUT_PWM_PIN, 235); // Zielwert für Stufe 1 (invertiert)
    ...
    case 2:                           // Zweite Stufe
      ledcWrite(OUTPUT_PWM_PIN, 220); //(invertiert)
    ...
    case 3:                           // Dritte Stufe
      ledcWrite(OUTPUT_PWM_PIN, 175); //(invertiert)
    ...
    case 4:                         // Vierte Drehstufe (Vollgas)
      ledcWrite(OUTPUT_PWM_PIN, 0); //(invertiert)
      currentDutyCycle = 0;
      boostAktiv = true; // Verriegelt lassen
      var_falsche_Stoerung = false;
      break;
  ...
  if (aktueller_zustandx == LUEFTER_AUS)
  {
    ...
    ledcWrite(OUTPUT_PWM_PIN, 255); //(invertiert)
    currentDutyCycle = 255;
    button_PWM_stage = -1;
  }
  ...
  // --------------------------------------------------------------------------------------
  // Hier stehen die Funktionsaufrufe, die an das Gateway des Webinterface übergeben werden
  // --------------------------------------------------------------------------------------
  ...
  // Keinen falschen Segmentwert-Ausreißer an Web-Interface übergeben(Abfangen): Stufe muss mindestens 100 ms stabil sein.
  ...
  // LED‑Werte an Statusanzeige senden(Gateway)
